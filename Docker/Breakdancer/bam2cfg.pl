...
use FindBin qw($Bin);
use lib "$FindBin::Bin";
use lib "/opt/conda/envs/Breakdancer/lib/breakdancer-maxunstable";
use AlnParser;

$| = 1; # enable AUTOFLUSH modE

my %opts = (q=>35, n=>10000, v=>1, c=>4, b=>50, s=>50, t=>0.01);
getopts('q:n:c:b:p:s:t:hmf:gCv:', \%opts);

die("ERROR: -t must be >= 0 and < 0.5\n")
  if (!defined $opts{t} || $opts{t} < 0 || $opts{t} >= 0.25);
...
Options:
         ...
         -t FLOAT  Trim distribution of insert sizes by this fraction from each tail to remove outliers when estimating mean and s.d. insert size [$opts{t}]
\n
") unless (@ARGV);
...
foreach my $fbam(@ARGV){
  ...
  foreach my $lib(keys %insert_stat){
    ...
    my @isize = sort { $a <=> $b } $insert_stat{$lib}->get_data();

    delete $insert_stat{$lib};
    my $insertsize=Statistics::Descriptive::Full->new();

    # skip libraries with less than 100 observations
    my $n = scalar @isize;
    next if($n < 100);

    # trim 1% from each tail of the sorted insert sizes to remove outliers
    
    ## Define start and end of sorted insert sizes based on trim fraction
    my $trim  = int($n * $opts{t});
    my $start = $trim;
    my $end   = $n - $trim - 1;
    next if($start > $end);

    ## do the tail trimming
    my @trimmed = @isize[$start .. $end];
    $insertsize->add_data(@trimmed);

    # calculate mean, s.d. and coefficient of variation (cv) of the insert sizes after trimming
    my $mean=$insertsize->mean();
    my $std=$insertsize->standard_deviation();
    next if($mean<$opts{s});
    my $cv=$std/$mean;
    if($cv>=$opts{v}){
    ...
    # Avoid division-by-zero without skipping the library.
    # Fallback: use symmetric SD from trimmed distribution.
    my $fallback_sd = $std;
    $fallback_sd = 1 if(!defined $fallback_sd || $fallback_sd <= 0);
